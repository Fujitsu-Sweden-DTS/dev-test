...
export function concatenateOrganizationalData(organizationNames, organizationLocations, organizationManagers) {
  let result = [];
  for (let orgName of organizationNames) {
    let orgLoc = organizationLocations.find((orgLoc) => orgLoc.id == orgName.id);
    let orgMan = organizationManagers.find((orgMan) => orgMan.id == orgName.id);
    result.push({ id: orgName.id, name: orgName.name, location: orgLoc.location, manager: orgMan.manager });
  }
  return result;
}
